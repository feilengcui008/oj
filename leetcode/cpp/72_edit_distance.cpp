  ...
  public:
    ...
    int minDistance(string word1, string word2)
    {
      ...
      for (int i = 1; i <= l1; ++i) {
        for (int j = 1; j <= l2; ++j) {
          // replace 
          t1 = (word1[i-1] == word2[j-1]) ? dp[i-1][j-1] : dp[i-1][j-1] + 1;
          // delete
          t2 = dp[i-1][j] < dp[i-1][j-1] ? dp[i-1][j] + 1 : dp[i-1][j-1] + 1;
          // insert
          t3 = dp[i][j-1] < dp[i-1][j-1] ? dp[i][j-1] + 1 : dp[i-1][j-1] + 1;
    ...
    int ithDistance(string word1, string word2, int i, int pre)
    {
      ...
      // replace a char
      int t2 = ithDistance(word1.substr(1, word1.length()-1), word2, i+1, pre+1);
      // delete a char
      int t3 = ithDistance(word1.substr(1, word1.length()-1), word2, i, pre+1);
      // insert a char
