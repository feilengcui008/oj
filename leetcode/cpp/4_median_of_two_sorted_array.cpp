...
class Solution {
  public:
    double findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2)
    {
      ...
      int len2 = nums2.size();
      if ((len1 + len2) % 2 != 0) {
        return findKthSortedArrays(nums1, len1, nums2, 
            len2, (len1 + len2 + 1) / 2);
      } else {
        return (findKthSortedArrays(nums1, len1, nums2, 
              len2, (len1 + len2) / 2) + findKthSortedArrays(nums1, len1, 
              nums2, len2, (len1 + len2) / 2 + 1 )) / 2.0;
      }
    }
  private:
    double findKthSortedArrays(vector<int> &num1, int leftlen1, 
        vector<int> &num2, int leftlen2, int k) 
    {
      ...
      if (k == 1) {
        return num1[num1.size() - leftlen1] < num2[num2.size() - leftlen2] ? 
          num1[num1.size() - leftlen1] : num2[num2.size() - leftlen2];
      }
      ...
      if (num1[leftindex] < num2[rightindex]) {
        return findKthSortedArrays(num1, leftlen1 - skip, num2, 
            leftlen2, k - skip);
      } else {
        return findKthSortedArrays(num1, leftlen1, num2, 
            leftlen2 - (k - skip), skip);
      }    
    }
};
