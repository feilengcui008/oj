 ...
 public:
  string multiply(string num1, string num2) {
    ...
    for (begin1 = 0; begin1 < len1; ++begin1) {
      ...
      if (::isdigit(num1[begin1]) && num1[begin1] != '0') break;
    }
    ...
    for (begin2 = 0; begin2 < len2; ++begin2) {
      ...
      if (::isdigit(num2[begin2]) && num2[begin2] != '0') break;
    }
    ...
    for (i = end1; i >= begin1; --i) {
      for (j = end2; j >= begin2; --j) {
        temp = (num2[j] - '0') * (num1[i] - '0') + res[end2 - j + end1 - i] -
               '0' + c;
        c = temp / 10;
        remain = temp % 10;
    ...
    auto ele = res.begin();
    while (ele != res.end() && (*ele) == '0') ele++;
    if (flag1 == flag2) return string(ele, res.end());
