...
class Solution {
  public:
    ...
    bool bfs(vector<vector<char>> &board, int i, int j, string &word, int k)
    {
      ...
      auto down = i < row - 1 && board[i+1][j] != '.' && board[i+1][j] == word[k+1];
      auto left = j > 0 && board[i][j-1] != '.' && board[i][j-1] == word[k+1];
      auto right = j < col - 1 && board[i][j+1] && board[i][j+1] == word[k+1];
      if (up) {
        auto b = board;
...
int main(int argc, char *argv[])
{
  ...
  vector<vector<char>> v22 { 
    {'a', 'b'},
    {'c', 'd'}
  };
  vector<vector<char>> v33 {
    {'A', 'B', 'C', 'E'},
    {'S', 'F', 'E', 'S'},
    {'A', 'D', 'E', 'E'}
  };
